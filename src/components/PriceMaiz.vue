<template>
    <v-container fill-height class="pb-10">
        ...
        <v-row wrap justify-center align-center class="primerrow" v-show="loading">
            <v-sheet
                elevation="12"
                max-width="600"
                rounded="lg"
            ...
            <v-col cols="12" lg="10" md="12" sm="12" xs="12">
                <v-row :wrap="true">
                    ...
                    <v-col cols="12" lg="6" md="6" sm="12" xs="12" v-if="showtitle" align="end">
                        ...
                        link
                        href="https://es.tradingview.com/symbols/CBOT-ZC1!/"
                        target="_blank"
                        >
                        TradingView.com
                        </v-btn>
  ...
  export default {
    ...
    data () {
      return {
        title: '',
        headers: [
          { text: '', align: 'start', value: 'mesano', class: ["subtitle-1","font-weight-bold"] },
          { text: '$/TM', value: 'valor', class: ["subtitle-1","font-weight-bold"] },
        ],
        desserts: [],
        loading: true,
        data1: '',
    ...
    created () {
        try {
            axios.get(process.env.VUE_APP_BASE_URL + 'infofuturos')
                .then(resp => {
                    let objeto = {
                        tags: [],
                        valores: [],
                    ...
                    resp.data.forEach(ele => {
                        if (ele.mesano === 'ACTUAL') {
                        ...
                        } else if (ele.mesano.substring(0,3) === 'ZCZ') {
                            this.desserts.push({mesano: 'Diciembre-' + ele.mesano.substring(3,ele.mesano.length), valor: ele.valor.toFixed(2)})
                            objeto.tags.push('Diciembre-' + ele.mesano.substring(3,ele.mesano.length))
                            objeto.valores.push(ele.valor.toFixed(2))
                        } else if (ele.mesano.substring(0,3) === 'ZCH') {
                            ...
                            objeto.valores.push(ele.valor.toFixed(2))
                        } else if (ele.mesano.substring(0,3) === 'ZCK') {
                            ...
                            objeto.valores.push(ele.valor.toFixed(2))
                        } else if (ele.mesano.substring(0,3) === 'ZCN') {
                ...
                .catch(error => {
                    ...
                    this.erroreslocales.message = 'Existe un error con petición'
